...
import { franchisee_document } from "@prisma/client";

export default async function AdminFranchiseeDocsPage() {
  ...
  const docs = await prisma.franchisee_document.findMany({
  ...
  return (
    <div className="space-y-8 max-w-6xl mx-auto pb-20">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">Franchisee Documents</h1>
          <p className="text-gray-500 mt-2">Manage legal agreements, certificates, and center-specific resources.</p>
        </div>
          ...
          <table className="w-full text-left font-medium">
            <thead>
              <tr className="bg-slate-50 border-b border-slate-100">
                <th className="px-8 py-4 text-xs font-bold text-slate-400 uppercase tracking-widest">Document Name</th>
                <th className="px-8 py-4 text-xs font-bold text-slate-400 uppercase tracking-widest text-center">Type</th>
                <th className="px-8 py-4 text-xs font-bold text-slate-400 uppercase tracking-widest">Uploaded By</th>
            ...
            <tbody className="divide-y divide-slate-100">
              {docs.map((doc: franchisee_document) => (
                <tr key={doc.doc_id} className="hover:bg-slate-50/50 transition-colors group">
                  <td className="px-8 py-6">
                    <div className="flex items-center gap-3">
                       <div className="w-10 h-10 bg-blue-50 rounded-xl flex items-center justify-center text-blue-600">
                          <FileText className="w-5 h-5" />
                       </div>
                       <span className="font-bold text-slate-800 capitalize group-hover:text-blue-600 transition-colors">{doc.document_name}</span>
                    </div>
                  </td>
